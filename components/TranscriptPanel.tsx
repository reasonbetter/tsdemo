...
export function TranscriptPanel({ history, currentTheta, capabilities }: { history: TranscriptEntry[]; currentTheta: DisplayTheta; capabilities?: { usesProbes?: boolean; continuousScore?: boolean } | null }) {
  ...
  return (
    <CollapsibleSection title="Transcript History" className="bg-card shadow-sm" titleSize="sm">
      ...
      {hasHistory ? (
        ...
        <div className="px-3 pb-3 text-sm text-muted-foreground italic">No transcript yet.</div>
      )}
    </CollapsibleSection>
  );
}
