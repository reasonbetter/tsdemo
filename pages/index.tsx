...
export default function Home() {
  ...
  // showDebug state is removed as the sections are now collapsible
  ...
  // (These functions remain the same as the previous implementation)
    ...
    <div className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
       <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold leading-tight tracking-tight text-foreground">
            Reasoning Demo — Causal Structure
        </h1>
       ...
       <div className="flex items-center gap-4 mb-6 p-4 bg-card border border-border rounded-xl shadow-sm">
            <label className="text-base font-medium text-foreground">User ID (optional)</label>
            <input
                className="w-48 px-3 py-2 text-base border border-input-border rounded-lg focus:ring-primary focus:border-primary transition duration-150"
                value={userTag}
                onChange={(e) => setUserTag(e.target.value)}
                placeholder="Enter ID"
            />
            <p className="text-sm text-muted-foreground ml-4 hidden md:block">
                Provide an ID to easily find this session in the admin logs later.
            </p>
        </div>

      {/* Session Info (Collapsible, Closed by Default) */}
      <CollapsibleSection title="Session Info" className="mb-6">
        <div className="flex flex-wrap gap-3">
            {/* Badge Styling */}
            <span className="inline-flex items-center gap-2 px-3 py-1 text-sm text-muted-foreground bg-background border border-border rounded-full"><strong>θ (Theta)</strong>&nbsp;{theta.mean}</span>
            <span className="inline-flex items-center gap-2 px-3 py-1 text-sm text-muted-foreground bg-background border border-border rounded-full"><strong>SE (Standard Error)</strong>&nbsp;{theta.se}</span>
            <span className="inline-flex items-center gap-2 px-3 py-1 text-sm text-muted-foreground bg-background border border-border rounded-full">Current Item: {currentItem.item_id}</span>
            ...
            <span className="inline-flex items-center gap-2 px-3 py-1 text-sm text-muted-foreground bg-background border border-border rounded-full">Session ID: {sessionId}</span>
        </div>
      </CollapsibleSection>


      {/* Main Assessment Card */}
      <section className="bg-card shadow-lg border border-border rounded-xl p-6 mb-6">
      ...
      {/* History Section (Collapsible, Closed by Default) */}
      {history.length > 0 && (
          <CollapsibleSection title="History" className="mb-6">
            <div className="space-y-8">
                ...
                <div key={`${h.item_id}-${index}`} className="border-t border-border pt-6 first:pt-0 first:border-t-0">
                    ...
                    <div className="text-muted-foreground mb-3 prose-sm">
                        <Prose>{h.text}</Prose>
                    </div>
       ...
       <CollapsibleSection title="Current Item Guidance (AJ)" className="mb-6">
       ...
       <CollapsibleSection title="Session Trace (Debug)">
